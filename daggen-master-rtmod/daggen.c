	...
	for (i=0; i<dag->nb_levels-1; i++) {
		for (j=0; j<dag->nb_tasks_per_level[i]; j++) {
			for (k=0; k<dag->levels[i][j]->nb_children; k++) {
				int ds = global.minTransfer + (rand()%(global.maxTransfer-global.minTransfer));
				int minDS = MIN(ds,MIN(dag->levels[i][j]->sendBudget, dag->levels[i][j]->children[k]->recvBudget));
				dag->levels[i][j]->dataSize[k] = minDS;
				dag->levels[i][j]->sendBudget -= minDS;
				dag->levels[i][j]->children[k]->recvBudget -= minDS;
...
static void generateDependencies(DAG dag) {
	...
	for (i=0; i<dag->nb_levels; i++) {
		for (j=0; j<dag->nb_tasks_per_level[i]; j++) {
			...
			dag->levels[i][j]->dataSize = (int *)calloc(dag->levels[i][j]->nb_children, sizeof(int));
...
static void generateTasks(DAG dag) {
	...
	for (i=0; i<dag->nb_levels; i++) {
		...
		for (j=0; j<dag->nb_tasks_per_level[i]; j++) {
			...
			dag->levels[i][j]->cpu = rand()%global.cpus;
			int wcet = global.minWCET + (rand()%(global.maxWCET-global.minWCET));
			dag->levels[i][j]->wcet = wcet;
			dag->levels[i][j]->period = MAX(global.minPeriod,wcet + (rand()%(global.maxPeriod-wcet)));
			dag->levels[i][j]->sendBudget = global.sendBudget;
			dag->levels[i][j]->recvBudget = global.recvBudget;
